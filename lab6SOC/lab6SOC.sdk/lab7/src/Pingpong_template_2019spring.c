...
void MoveBallRight(void) {
	...
	//keep the score add once
	int getOut = 1;

	for (led_order = LED_PATTERNS_ORDER[5]; led_order >= LED_PATTERNS_ORDER[0];) {
...
void MoveBallLeft(void) {
	...
	//keep the score add once
	int getOut = 1;
	for (led_order = LED_PATTERNS_ORDER[1]; led_order <= LED_PATTERNS_ORDER[5];) {
		run = RUNKEEP;
		while (run) {
			dip_check = XGpio_DiscreteRead(&dip, 1);
			if (dip_check != dip_check_prev) {
			...
			if (psb_check == RIGHTPADDLE) {
				...
				if (led_order == LED_PATTERNS_ORDER[5] && getOut) {
					//set GameOver; display scores
					scoreright += 1;
					GameOver = STOP;
					StartDirection = LEFT;
